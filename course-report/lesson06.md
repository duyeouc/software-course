...
- 使用GDAL编程，对图像进行分块后进行图像融合
- 使用两种方式：$256*256$，$图像宽度*256$
- 比较两种方式的运行效率
...
实验得到了两种方式的融合图像，截图如下所示：

![](http://ww1.sinaimg.cn/large/00632YXjly1fxpb79yybpj30ls0gp1kx.jpg)

![](http://ww1.sinaimg.cn/large/00632YXjly1fxpb7kx387j30lq0go1kx.jpg)

实验记录了两种方式的运行时间，第一种方式约为$87s$，第二种方式约为$46s$。可见，第二种方式分块效率高。

### 5.其他问题

实验中要注意的问题是，图像大小并不是256的整数倍，因此进行每行/列最后一块处理时，要单独计算分块大小，对此分块载入内存进而进行处理。